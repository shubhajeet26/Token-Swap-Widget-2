...
export const CONTRACT_ADDRESS =
  "CD3OBFYUTTK4MYD7ATN3CESOJ2HWZAYH7452VCXIKGP47JAYVJOQDAN";
 ...
 * Calls: create_swap(creator, offer_token, offer_amount, request_token, request_amount) -> u32
 */
export async function createSwap(
  ...
  return callContract(
    ...
    [
      ...
      toScValAddress(offerToken),
      toScValI128(offerAmount),
      toScValAddress(requestToken),
      toScValI128(requestAmount),
    ],
    caller,
    true
  );
 ...
 * Calls: accept_swap(acceptor, swap_id)
 */
export async function acceptSwap(caller: string, swapId: number) {
  return callContract(
    "accept_swap",
    [toScValU32(swapId)],
    caller,
 ...
 * Calls: cancel_swap(caller, swap_id)
 */
export async function cancelSwap(caller: string, swapId: number) {
  return callContract(
    "cancel_swap",
    [toScValU32(swapId)],
    caller,
 ...
 * Calls: get_swap(swap_id) -> SwapOffer
 */
export async function getSwap(swapId: number, caller?: string) {
  return readContract("get_swap", [toScValU32(swapId)], caller);
}

/**
 * Get all swaps.
 * Calls: get_all_swaps() -> Vec<SwapOffer>
 */
export async function getAllSwaps(caller?: string) {
  return readContract("get_all_swaps", [], caller);
}

/**
 * Get only active swaps.
 * Calls: get_active_swaps() -> Vec<SwapOffer>
 */
export async function getActiveSwaps(caller?: string) {
  return readContract("get_active_swaps", [], caller);
}
